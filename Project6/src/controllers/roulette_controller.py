...
class rouletteController(AbstractController):
    ...
    def execute(self):
        ...
        while(True):
            ...
            if bet_string == "spin":
                ...
                earningCaculateView(
                    earn - loss, self.player.user_wallet).render()
                self.player.update()
                self.bets = []
            ...
            betPutView([(x.bet_money, x.bet_string)
                       for x in self.bets]).render()
    ...
    def calculate_bet(self, spin_result):
        ...
        for bet in self.bets:
            if bet.fit(result_number, result_color):
                earn += bet.bet_money * \
                    self.roulette_table.rate_table[bet.bet_string]
            else:
                loss += bet.bet_money
        return earn, loss
    ...
    def validate_input_format(self, s):
        return " " in s and s.split(
            " ")[1] in self.roulette_table.rate_table.keys() and s.split(" ")[0].isnumeric()

    def update_state(self):
        raise NotImplementedError(
            'Controller has not defined method update_state()')
...
class rouletteTable:
    def __init__(self):
        ...
        self.rate_table.update(dict.fromkeys(
            ['1-18', '19-36', 'even', 'odd', 'red', 'black'], 2))
        self.spin_table = [
            (x, 'red') if x in [
                1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36] else (
                x, 'black')for x in range(
                1, 37)]
