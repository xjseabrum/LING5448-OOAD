...
class GameView(AbstractView):
    ...
    def render(self, **kwargs):
        ...
        prefix = "PLAYER {}-> ".format(kwargs['player_num']
                                       ) if 'player_num' in kwargs else self.prefix
        ...
        if 'method' in kwargs:
            if 'acceptable_inputs' in kwargs:
                return display_msg(kwargs['method'], prefix + kwargs['message'], animation_delay,
                                   leading_elipsis, acceptable_inputs=kwargs['acceptable_inputs'])

            else:
                return display_msg(
                    kwargs['method'], prefix + kwargs['message'], animation_delay, leading_elipsis)
